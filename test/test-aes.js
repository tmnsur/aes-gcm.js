...
describe("aes", function() {
  ...
  describe("#decrypt()", function() {
    it("should decrypt: AES-128", function() {
      assert.deepEqual(hex.fromHex("00112233445566778899aabbccddeeff"),
        aes.decrypt(hex.fromHex("69c4e0d86a7b0430d8cdb78070b4c55a"), hex.fromHex("000102030405060708090a0b0c0d0e0f")));
    });

    it("should decrypt: AES-192", function() {
      assert.deepEqual(hex.fromHex("00112233445566778899aabbccddeeff"),
        aes.decrypt(hex.fromHex("dda97ca4864cdfe06eaf70a0ec0d7191"),
          hex.fromHex("000102030405060708090a0b0c0d0e0f1011121314151617")));
    });

    it("should decrypt: AES-256", function() {
      assert.deepEqual(hex.fromHex("00112233445566778899aabbccddeeff"),
        aes.decrypt(hex.fromHex("8ea2b7ca516745bfeafc49904b496089"),
          hex.fromHex("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f")))
    });

    it("should decrypt", function() {
      assert.deepEqual(hex.fromHex("00000000000000000000000000000000"),
        aes.decrypt(hex.fromHex("66e94bd4ef8a2c3b884cfa59ca342b2e"), hex.fromHex("00000000000000000000000000000000")));
      assert.deepEqual(hex.fromHex("00000000000000000000000000000000"),
        aes.decrypt(hex.fromHex("c6a13b37878f5b826f4f8162a1c8d879"), hex.fromHex("000102030405060708090a0b0c0d0e0f")));
      assert.deepEqual(hex.fromHex("00000000000000000000000000000000"),
        aes.decrypt(hex.fromHex("73a23d80121de2d5a850253fcf43120e"), hex.fromHex("ad7a2bd03eac835a6f620fdcb506b345")));
    })
  });
});
